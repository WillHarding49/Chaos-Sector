...
AlienRing::AlienRing( sf::Texture& texture, sf::Vector2f& scale, sf::Vector2f& pos, sf::Vector2f& defaultPos, float speed, const std::vector<std::vector<sf::Vector2f>>& path )
...
void AlienRing::update( const float& deltaTime, const sf::Vector2f& playerPos, ProjectilePool& missilePool )
{
	Enemy::update( deltaTime, playerPos, missilePool );

}

void AlienRing::dive( const float& deltaTime )
{
